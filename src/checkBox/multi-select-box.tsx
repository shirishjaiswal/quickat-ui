...
export const MultiSelectBox: React.FC<MultiSelectBoxProps> = ({
  ...
  const handleSingleSelect = (idx: number) => {
    ...
    setOptions(newValue);
    const event = getEventFormat(newValue, getError()) as MultiSelectBoxOnChangeEvent;
    onTick(event);
  };
      ...
      <div id="cb-container-list" className={`${boxContainerStyles}`}>
        {cbOptions.map((option, idx) => (
          <div className="flex items-center gap-2">
            ...
            <label className={`${optionStyle} ${option.required ? 'after:content-["*"] after:text-rose-700 after:ml-1' : ''} `}>{option.label}</label>
          </div>
        ))}
      </div>
      {errorMessage && (
